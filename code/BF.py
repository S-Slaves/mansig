async def brainFuck(message):
    brain = '{0}'.format(message.content[4:])
    ...
    while curloca != length_total:
        ...
        if error == 100:
            ...
            # 현재 주소가 0이라면 에러 생성(3, Null Pointer Exception)
            if curdata == 0:
                error = 3
                break
            else:
            ...
            if data[curdata] != 0:
                ...
                if loop_count > 10000000:
                    # 천만 번 이상 돌면 에러 생성(10000, Infinite loop)
                    error = 10000
                    break
            else:
                loop_count = 0
            ...
            await message.channel.send("','가 발견되었습니다. 아스키 문자를 입력해주세요.")
            while True:
                # 아스키 문자를 입력하지 않았거나 2개 이상 입력하면 재입력 요구
                asc = '{0}'.format(input())
                if len(asc) != 1:
                    await message.channel.send("잘못된 입력입니다. 아스키 문자를 입력해주세요.")
                else:
                    break
            if ord(asc) < 127 and ord(asc) >= 32:  # ASCII 코드가 맞을 경우
                # 현재 위치에 입력받은 문자의 ASCII 코드 입력
                data[curdata] = ord(asc)
            # 아닐 경우 에러 생성(1, Unable to Print/Save)
            else:
                error = 1
            ...
            if data[curdata] < 127 and data[curdata] >= 32:   # ASCII 코드가 맞을 경우
                ...
                error = 1
                # 아닐 경우 에러 생성(1) 및 에러 주소 출력
                error_address = f'{point_number}'
                break

        curloca += 1
    if error == 10000:
        await message.channel.send("무한루프가 의심되어 작업을 정지합니다.")
    elif error == 100:
        await message.channel.send("괄호가 제대로 처리되어 있지 않습니다.")
    elif error == 3:
        await message.channel.send("포인터 번호가 0보다 작습니다.")
    elif error == 1:
        await message.channel.send("입력/출력 가능 ASCII가 아닙니다.\n오류 출력 주소: " +
              error_address + '\n{0}'.format(data))
    else:
