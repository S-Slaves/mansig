...
braille_kor_JONG = {'ㄱ': '⠁', 'ㄳ': '⠁⠄', 'ㄴ': '⠒', 'ㄵ': '⠒⠅', 'ㄶ': '⠒⠴', 'ㄷ': '⠔', 'ㄹ': '⠂', 'ㄺ': '⠂⠁', 'ㄻ': '⠂⠢',
                    'ㄼ': '⠂⠃',
                    'ㄽ': '⠂⠄', 'ㄾ': '⠂⠦', 'ㄿ': '⠂⠲', 'ㅀ': '⠂⠴', 'ㅁ': '⠢', 'ㅂ': '⠃', 'ㅄ': '⠃⠄', 'ㅅ': '⠄', 'ㅇ': '⠶',
                    'ㅈ': '⠅', 'ㅊ': '⠆', 'ㅋ': '⠖', 'ㅌ': '⠦', 'ㅍ': '⠲', 'ㅎ': '⠴', 'ㄲ': '⠁⠁', 'ㅆ': '⠌',
                    '': ''}  # 종성 점자 리스트 (한·점 제1장 제4항~제6항)
abbr_list_cho_cj = ['ㄱ', 'ㄴ', 'ㄷ', 'ㅁ', 'ㅂ', 'ㅅ',
                    'ㅈ', 'ㅋ', 'ㅌ', 'ㅍ', 'ㅎ']  # 초성 + ㅏ 꼴 약자 리스트 - 초성
abbr_list_braille_cj = ['⠫', '⠉', '⠊', '⠑', '⠘', '⠇',
                        '⠨', '⠋', '⠓', '⠙', '⠚']  # 초성 + ㅏ 꼴 약자 리스트 - 점자
abbr_list_jung_jj = ['ㅓ', 'ㅓ', 'ㅓ', 'ㅕ', 'ㅕ', 'ㅕ', 'ㅗ', 'ㅗ',
                     'ㅗ', 'ㅜ', 'ㅜ', 'ㅡ', 'ㅡ', 'ㅣ']  # 중성 + 종성 꼴 약자 리스트 - 중성
abbr_list_jong_jj = ['ㄱ', 'ㄴ', 'ㄹ', 'ㄴ', 'ㄹ', 'ㅇ', 'ㄱ', 'ㄴ',
                     'ㅇ', 'ㄴ', 'ㄹ', 'ㄴ', 'ㄹ', 'ㄴ']  # 중성 + 종성 꼴 약자 리스트 - 종성
...
class NormalLetter:  # 한글이 아닌 문자 객체
    def __init__(self, thisnamewillnotbeused, num):
...
def abbreviation(cho, jung, jong, repl, mode='jj'):  # 점자 약자 변환을 위한 함수(한·점
    ...
    if mode == 'cj':  # 초성 + 중성 약자
        ...
        if letter.cho in ['ㅅ', 'ㅈ', 'ㅊ', 'ㅆ', 'ㅉ'] and letter.jung == 'ㅕ' and letter.jong == 'ㅇ':
            # 성, 정, 청, 썽, 쩡은 기존 ᅟᅧᆼ의 약자를 사용하여 표기, 셩, 졍, 쳥, 쎵, 쪙은 약자 사용하지 않음 (한·점 제16항)
            pass
        else:
...
def braille_convert(message):
        ...
        if type(letter) == NormalLetter:
            ...
            if letter.list.count('') == 2:
                if letter.cho == '':
                    ...
                    letter.braille.append('⠿')
                    letter.cho_braille = braille_kor_CHOJUNG[
                        letter.cho]  # 자음이나 모음이 단독으로 쓰일 경우 그 앞에 온표 ⠿를 붙여 나타냄 (한·점 제9항)
                    if letter.cho == 'ㅇ':
                        letter.cho_braille = '⠛'
            else:
    ...
    for letter in each_letter_list:  # 예외 적용
        if type(letter) == NormalLetter:
            ...
            if letter.num != 0:
                if letter.jung == 'ㅖ' and letter.cho == 'ㅇ' and each_letter_list[letter.num - 1].jong == '' and \
                        each_letter_list[letter.num - 1].jung != '':
                    # 모음자에 '예'가 이어 나오면 그 사이에 붙임표 ⠤을 넣음 (한·점 제10항)
                    letter.braille.append('⠤')
                a = each_letter_list[letter.num - 1].jung
