...
async def embedsend(message):
    if len(message.content.split()) == 1:
        ...
        await explain(message, '시간', '만식이가 현재 시간을 time.ctime()을 사용하여 알려줍니다.', '`!시간`',
                      '문자열, 지금 시각은 (시간)래유~')
        await explain(message, 'QR', '만식이가 주어진 문자열에 대한 QR코드를 생성합니다.', '`!QR (변환하고 싶은 문자열)`',
                      '사진, QR코드')
        await explain(message, '바코드', '만식이가 주어진 12자리의 숫자에 대한 바코드를 생성합니다.', '`!바코드 (12자리의 숫자)`',
                      '사진, 바코드')
        await explain(message, '모스부호', '만식이가 주어진 문자열을 모스부호로 변환합니다. 영문과 한글, 숫자와 기호를 지원합니다.', '`!모스부호 (변환하고 싶은 문자열)`',
                      '문자열, 모스부호(—와 •으로 이루어짐)')
        await explain(message, '점자', '만식이가 주어진 한글 및 기호에 대해 점자 변환을 합니다.', '`!점자 (변환하고 싶은 문자열)`',
                      '문자열, 점자')
        await explain(message, '점자변환', '만식이가 주어진 점자를 숫자로(1~6점), 숫자를 점자로 변환합니다.', '`!점자변환 -숫자로 (점자)`\n`!점자변환 -점자로 (스페이스로 구분된 1에서 8까지의 숫자)',
                      '문자열, 점자 또는 숫자')
        await explain(message, 'md5배틀', '만식이가 둘 사이에 싸움을 붙입니다. -자세히 옵션을 통해 방어력, 크리티컬 확률, 순발력까지 확인 가능합니다. -생중계 옵션(관리자 전용)은 생중계를 지원합니다.', '`!md5배틀 (문자열 1) (문자열 2)`\n`!md5배틀 -자세히 (문자열 1) (문자열 2)`\n`!md5배틀 -생중계 (문자열 1) (문자열 2)`',
                      '문자열, 각 문자열의 체력/공격력/민첩성(+방어력/크리티컬률/순발력, -자세히 옵션 설정 시)\n싸우는 과정(-생중계 옵션 설정 시)\n승자')
        await explain(message, '지뢰찾기', '만식이가 지뢰찾기 게임을 만듭니다.')