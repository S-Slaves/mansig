...
async def embedsend(message):
    if len(message.content.split()) == 1:
        ...
        embed.add_field(name="__제작자 전용__", value="`테러`, `검열`, `검열중지`, `무한검열`, `무한검열중지`", inline=False)
        ...
        await explain(message, '\\s', "검색봇이 당신의 명령을 지우고 이를 대신 출력합니다.", "`\\s[출력할 내용]` (띄어쓰기 없음)",
                      "문자열, [출력할 내용]")
        await explain(message, '출력', "검색봇이 당신의 메시지를 그대로 출력합니다.", "`s출력 [출력할 내용]`",
                      "문자열, [출력할 내용]")
        await explain(message, '안녕', "검색봇이 당신의 인사를 받아줍니다.", "`s안녕`",
                      "문자열, 안녕하세요!")
        await explain(message, '핑', "검색봇이 discord.Client().latency를 사용하여 핑을 출력합니다.", "`s핑`",
                      "문자열, [핑 시간]ms!")
        await explain(message, '도움', "검색봇이 당신에게 명령어에 대한 도움말을 줍니다.", "`s도움 [명령어]`\n`s도움`",
                      "임베드")
        await explain(message, '채널', '검색봇이 이 서버에 있는 모든 채널들의 목록을 출력합니다.', '`s채널`',
                      '임베드')
        await explain(message, '프사', '검색봇이 당신의 프사를 출력합니다.', '`s프사`',
                      '사진, 당신의 프로필 사진')
        await explain(message, '유닉스시간', '검색봇이 1970년 1월 1일부터 지금까지 흘러온 초 수를 측정해 보여줍니다.', '`s유닉스시간`',
                      '숫자, UNIX 시간(소수점 아래 7자리까지)')
        await explain(message, '달력', '검색봇이 한 해나 한 달의 달력을 출력합니다.', '`s달력 [연도]`\n`s달력 [연도] [달]`',
